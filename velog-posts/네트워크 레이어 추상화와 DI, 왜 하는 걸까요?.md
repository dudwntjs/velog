...
<h2 id="배고픈-controller가-식당에-갑니다">배고픈 Controller가 식당에 갑니다</h2>
...
<hr />
<h2 id="메뉴판api-protocol">메뉴판(API Protocol)</h2>
<pre><code class="language-swift">protocol UserServiceProtocol {
    func getUser(id: Int) async throws -&gt; User
}</code></pre>
<h3 id="💬-이건-뭐냐면">💬 이건 뭐냐면?</h3>
<ul>
...
<h2 id="레시피-카드targettype">레시피 카드(TargetType)</h2>
...
<hr />
<h2 id="가게-주인service가-요리사provider에게-이-레시피대로-만들어달라고-부탁">가게 주인(Service)가 요리사(Provider)에게 '이 레시피대로 만들어달라'고 부탁</h2>
...
<hr />
<h2 id="요리사provider는-배달기사urlsession에게-장-봐와라고-시킴">요리사(Provider)는 배달기사(URLSession)에게 '장 봐와!'라고 시킴</h2>
...
<hr />
<h2 id="요리사provider가-재료data를-손질해서-요리를-만듦decode">요리사(Provider)가 재료(Data)를 손질해서 요리를 만듦(Decode)</h2>
...
<hr />
<h2 id="가게-주인service이-플레이팅dto-→-domain-model">가게 주인(Service)이 플레이팅(DTO → Domain Model)</h2>
...
<hr />
<h2 id="손님controller은-완성된-음식user을-받음">손님(Controller)은 완성된 음식(User)을 받음</h2>
...
<hr />
<h2 id="네트워크-통신-추상화-주방을-숨기는-설계">네트워크 통신 추상화: 주방을 숨기는 설계</h2>
<h3 id="👎-추상화-안-한-경우">👎 추상화 안 한 경우</h3>
<ul>
...
<hr />
<h2 id="di의존성-주입-가게-주인을-갈아끼우는-주문-시스템">DI(의존성 주입): 가게 주인을 갈아끼우는 주문 시스템</h2>
...
<p>손님은 ‘돈카츠 하나요’만 말하고, 
주인–요리사–배달이 알아서 처리해 완성된 요리만 받는 구조 </p>
<h3 id="-잘-추상화된-네트워크-레이어라고-볼-수-있습니다">= 잘 추상화된 네트워크 레이어라고 볼 수 있습니다</h3>
<hr />
